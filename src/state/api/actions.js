...
export const loadFacets = () => (dispatch, getState) => {
    ...
    setTimeout(() => {
        ...
        dispatch(loadNewFacetResults(dummyFacetResults));
    }, 0)
}
...
export const newSearch = newQueryString => (dispatch, getState) => {
    ...
    // Might also want to check a timestamp here for staleness
    if(newQueryString === currentSearchQueryString) {
        ...
        // Everything is already hunky dory and just jump to the bottom and redirect to 
        // search results if not already on that page (this would stop extra refreshes)

        // Behavior here is different depending on the location
        // If it's the search page it needs to redirect to results
        // If it's results already this 
        // This might be better to do in the component to avoid an extra refresh
    }
    if(isCached) {
    ...
    // Set new results in the current slots (could also refactor to have currentresults just be
    // a pointer to the cachedResults map)
    // Need to have a size limit on the cache (either number of records or memory footprint (which
    ...
    setTimeout(() => {
        const resultsToCache = {
            [newQueryString]: dummyResults,
        };

        // Process facets into filterstate
        const rawFacets = dummyResultsOriginal.facets;
        const facets = rawFacets.reduce((acc, facetType) => {
            ...
            acc[facetType.param] = facetTypeFilters;
            return acc;
        }, {})


        dispatch(cacheNewSearchResults(resultsToCache));
        dispatch(loadNewSearchResults(dummyResults));
    ...
    // We want to redirect even if we are already on the search results page so that users are able to
    // step back through previous searches
    // Set current searchQueryString to currentSearchQueryString

    
    // Last, redirect to search results page if not already on the search results page
    // TODO: Determine this logic. Two search action creators or just a flag? (Well, we want to effectively
    // reload when a new search is executed so hmmm)

    // Somewhere in here we need to normalize the data to go from an array of facets to a hashmap
}
